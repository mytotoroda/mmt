...
export default function CreateAta({ campaignId }: { campaignId: string }) {
  const [status, setStatus] = useState<AtaStatus | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const response = await fetch(`/api/airdrop/statusata/${campaignId}`);
  ...
  const createAta = async () => {
    try {
      ...
      const response = await fetch(`/api/airdrop/createata/${campaignId}`, {
        method: 'POST'
      });

      if (!response.ok) {
        throw new Error('ATA 생성 중 오류가 발생했습니다.');
      }
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-4 text-center mb-6">
        <div>
          <h4 className="text-sm text-gray-500 dark:text-gray-400">생성 완료</h4>
      ...
      <button
        className={`w-full py-3 px-4 rounded-lg font-medium text-white transition-colors
          ${isCreating || status.pending_count === 0
            ? 'bg-gray-400 cursor-not-allowed'
            : 'bg-blue-600 hover:bg-blue-700'
          }`}
        onClick={createAta}
        disabled={isCreating || status.pending_count === 0}
      >
        {isCreating ? (
